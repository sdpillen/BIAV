...
Most of my longform notes will be in the form of triple quotes as seen here.  However, this is the interface
for the BrainAmp system run the BIAV experiment, itself. -Steven Pillen (SP)'''
    ...
    # Marker class for storing marker information
        ...
        while not finish:
            ...
            if msgtype == 1:
                ...
                -SP'''                
                counter = 0
                if BIAV_SubjectInterface.Record == True:
                    for item in data:
                        counter = counter+1
                        f.write(str(item))
                        f.write(',')
                        if counter == 32:
                            if self.TMS_Mark == True or BIAV_SubjectInterface.TMS_Mark == True:
                                print 'Event Marked'
                                self.TMS_Mark = False
                                BIAV_SubjectInterface.TMS_Mark = False
                                self.ser.write('0')
                            ...
                            elif BIAV_SubjectInterface.LeftTag == True:
                                f.write('7')
                                print 'left...tagged in recording!'
                                BIAV_SubjectInterface.LeftTag = False
                            elif BIAV_SubjectInterface.RightTag == True:
                                print 'right...tagged in recording!'
                                f.write('8')
                                BIAV_SubjectInterface.RightTag = False
                            else:
                                f.write('0')
                            f.write('\n')    
                ...
                if len(data1s) > channelCount * 1000000 / samplingInterval:
                    ...
                    BIAV_SubjectInterface.SPTruVal = np.average(density[13:16]) - np.average(density2[13:16]) # - np.average(density2[13:16])
                    BIAV_SubjectInterface.LoNoise = density[1] #1 Hz
                    BIAV_SubjectInterface.HiNoise = np.average(density[40:60]) #40-59 Hz
                    BIAV_SubjectInterface.VoltMax = np.amax(SMRTimeSeries)
                    BIAV_SubjectInterface.VoltMedian = np.median(SMRTimeSeries)
                    BIAV_SubjectInterface.VoltMin = np.amin(SMRTimeSeries)
                    BIAV_SubjectInterface.SMRTimeSeries = SMRTimeSeries
                    BIAV_SubjectInterface.SMRTimeSeries2 = SMRTimeSeries2
                    BIAV_SubjectInterface.AlphaSeries = AlphaSeries
                    BIAV_SubjectInterface.SMRDens = density[8:19]
                    BIAV_SubjectInterface.SMRDens2 = density2[8:19]
                    BIAV_SubjectInterface.AlphaDensity = densityA
                    ...
                    if BIAV_SubjectInterface.stage == 1 and BIAV_SubjectInterface.pausetime == False:
                            Alphas = np.vstack((Alphas, densityA[6:16]))
                            #print 'alphas are ', Alphas
                            BIAV_SubjectInterface.Alphas = Alphas
                            ...
                            if len(Alphas) > 2:
                                BIAV_SubjectInterface.Alphas = Alphas
                                BIAV_SubjectInterface.densityA = densityA
...
sorts of GUIs, was to make it as idiot proof as possible. However, since there are fewer degrees of freedom in this panel compared
to the NFT, I did not put as many of the step-by-step controls as in the NFT.  -SP '''
class testGUI(wx.Frame): 
    def __init__(self): 
        ...
        self.buttonToggleAlpha = wx.Button(panel, -1, label="Toggle Alpha/Mu", pos=(30,240))
        ...
        I set it up so that the buttons are enabled as appropriate, and at certain points, previous buttons
        that handle sensitive variables (such as whether TMS control is active) are disabled.  I wanted
        this process to be as linear as possible, though here, as mentioned before, we are looser than
        with the main NFT.  -SP'''        
    ...
    def Filename(self, event):
        ...
        if filename != None:
            ...
            BIAV_SubjectInterface.filename = fname
            self.buttonConnect.Enable()
            self.Refresh()
            self.Update()
        BIAV_SubjectInterface.OutputFilename = fname[:-4] + 'MetaData.csv'
        BIAV_SubjectInterface.TS = fname[:-4] + 'TimeStamps.csv'
        BIAV_SubjectInterface.ExperimentOutputName = fname[:-4] + 'ContRec.csv'
        #self.update_NFT()
        BIAV_SubjectInterface.CustomName = True
    ...
    def ToggleAlpha(self, event):
        BIAV_SubjectInterface.alpha = not BIAV_SubjectInterface.alpha
        print 'Alpha is set to ', BIAV_SubjectInterface.alpha
        if BIAV_SubjectInterface.alpha:
    ...
    '''This initializes the main NFT event.'''
    def Connect(self, event):
        self.buttonToggleAlpha.Enable()
        BIAV_SubjectInterface.main()
        #self.the_thread = testThread(self)
        
    '''This governs both the shifting of rounds, and the changing of the instruction text displayed on the screen.
    It is important not to hit this button rapidly, thus the time.sleep function to avoid hamhanded misclicks. -SP'''
    def NextRound(self, event):
        BIAV_SubjectInterface.NEXT = True
        BIAV_SubjectInterface.RecordBypass = True
        BIAV_SubjectInterface.Submit = True
        self.buttonToggleAlpha.Disable()
        time.sleep(1)
